...
| DR-001 | RV1 Campaign (2026-07) | Broker reconciliation counts `EXPIRED` execution_registry entries with `broker_ticket = NULL` as `MISSING_BROKER`, keeping `ready_to_resume` permanently `false` | P1 | CLOSED | Codex | PATCH-DR001A | Fixed by excluding terminal `EXPIRED` / `CANCELLED` entries without `broker_ticket` from `missing_broker_count`; regression verified by `tsp_v2.tests.test_broker_reconciliation` and `unittest discover tsp_v2.tests` |
