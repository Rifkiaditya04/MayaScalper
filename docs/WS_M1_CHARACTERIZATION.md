...
| --- | --- | --- | --- | --- |
| 40 raw, 40 closed | PASS | Observed on `2026-06-22T03:19:08Z` for `GBPJPY` with `closed_bar_count.M1 = 40`. | No controlled replay for the same symbol/time. | Observed PASS |
| 40 raw, 39 closed | PASS | Observed on `2026-06-22T02:30:45Z` for `GBPUSD` and again on `2026-06-22T03:19:08Z`. | No raw dump for the passing cases. | Observed PASS |
| 40 raw, future bars | INCONCLUSIVE | Observed in failure cases with `future_count = 15` and `future_count = 9`. | Need a controlled replay to know whether future bars are causal or incidental. | Hypothesis open |
