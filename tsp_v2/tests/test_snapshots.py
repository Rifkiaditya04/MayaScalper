...
class M1FutureBarProvider(FakeMarketProvider):
    def get_rates(self, symbol: str, timeframe: str, count: int) -> list[dict[str, object]]:
        if timeframe != "M1":
            return super().get_rates(symbol, timeframe, count)
        del symbol
        step = 1
        last_time = self._tick_time + timedelta(minutes=2)
        start = last_time - timedelta(minutes=step * (count - 1))
        bars: list[dict[str, object]] = []
        current = start
        for idx in range(count):
...
class SnapshotTests(unittest.TestCase):
    ...
    def test_snapshot_emits_m1_raw_bar_dump_with_stats_when_m1_closed_bars_are_insufficient(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with self.assertRaises(ConfigValidationError):
                build_market_snapshot(
                    ...
                    symbol="XAUUSD",
                    cycle_time_utc=cycle_time,
                    build_config=SnapshotBuildConfig(m1_bars=34, m5_bars=71),
                    diagnostics_hook=diagnostics.append,
                )
        self.assertTrue(diagnostics)
        payload = diagnostics[-1]
        ...
        self.assertEqual(payload["requested_bars"], 34)
        self.assertEqual(payload["returned_bars"], 34)
        self.assertEqual(payload["closed_bar_count"], 31)
        self.assertEqual(payload["m1_raw_bar_stats"]["closed_count"], 31)
        self.assertEqual(payload["m1_raw_bar_stats"]["forming_count"], 1)
        self.assertEqual(payload["m1_raw_bar_stats"]["future_count"], 2)
        self.assertEqual(payload["m1_raw_bar_stats"]["duplicate_close_time_count"], 0)
